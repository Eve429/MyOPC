...
def signed_distance_initialization(target_u8: np.ndarray) -> np.ndarray:
    """从 query transmission 栅格生成前景为负的像素中心精确 SDF。
    ...
    if bool(binary.all()):
        return np.full(
            (height, width), -float(max(height, width)), np.float32)
    if not bool(binary.any()):
        return np.full(
            (height, width), float(max(height, width)), np.float32)
    # 顺序执行 outside/inside EDT 并即时释放 float64 中间量：初始化峰值
    # 不超过一份 float64 距离场 + 一份 float32 结果（REQ-003 SHOULD）。
    phi = distance_transform_edt(~binary).astype(np.float32)
    inside = distance_transform_edt(binary)  # float64 临时，用后即弃
...
def macro_gradient_magnitude(problem: PixelMacroProblem,
                             ...
                             macro_phi: np.ndarray) -> np.ndarray:
...
def optimize_levelset_macro(
    ...
    phi 常驻 CPU（[Hm,Wm] float32，Adam 状态同域）；每 backward state
    ...
    initial_query_phi = signed_distance_initialization(problem.target_u8)
