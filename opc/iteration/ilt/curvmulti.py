...
def _resize(image: torch.Tensor, shape: tuple[int, int], mode: str) -> torch.Tensor:
    """保持 `[B,H,W]` 契约缩放目标、参数或优化窗口。"""
    return functional.interpolate(image[:, None], size=shape, mode=mode)[:, 0]


def _curved_mask(parameters: torch.Tensor, config: CurvMultiConfig) -> torch.Tensor:
    """对连续参数做固定均值平滑和带偏移 sigmoid，生成可微软掩膜。"""
    padding = config.smoothing_kernel // 2
    pooled = functional.avg_pool2d(
        parameters[:, None], config.smoothing_kernel, stride=1,
        padding=padding)[:, 0]
    return torch.sigmoid(
        config.sigmoid_steepness * (pooled - config.sigmoid_offset))
...
def optimize_curvmulti(
    ...
    fixed_full_mask = _curved_mask(initial, config).detach()
    ...
    for scale in config.scales:
        ...
        stage_reference = _resize(initial, shape, "area")
        ...
        stage_initial = (stage_reference if previous_parameters is None else
                         _resize(previous_parameters, shape, "nearest"))
        movable = _resize(full_movable, shape, "nearest")
