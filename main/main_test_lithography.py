...
import argparse  # 命令行解析（flag 数量已超手写 argv 阈值，argparse 自带 --help）
import sys  # 仓库根路径引导（免安装直接运行）
from decimal import Decimal, InvalidOperation  # nm→DBU 精确十进制换算
...
from common.units import exact_dbu  # nm→DBU 精确换算（报错自带参数名）
from layout import LayerNotFoundError, LayerSpec, LayoutDB  # 版图打开与层规格
from lithography import ICCAD13Lithography  # 固定 ICCAD13 光刻模型
from main._macro_pipeline import save_lithography_pngs  # 与迭代管线同款留档内核
from opc.input import MaskPolarity  # 掩模极性枚举
...
def _layer_argument(text: str) -> LayerSpec:
    ...
    parts = text.split("/")
    if (len(parts) != 2
            or not all(part.strip().lstrip("-").isdigit() for part in parts)):
...
def _parse_arguments(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="GDS → 光刻结果留档：逐 tile nominal/binary PNG + manifest"
                    "（与迭代管线 final_lithography 产物同款）")
    parser.add_argument("gds", type=Path, help="输入 GDS 文件路径")
    parser.add_argument("--top", default=None, metavar="CELL",
                        help="顶层 Cell 名（缺省=版图唯一顶层；多顶层必须指名）")
    parser.add_argument("--layer", type=_layer_argument, default="11/0",
                        help="目标层 layer/datatype（默认 11/0，TestReticle 惯例）")
    parser.add_argument("--polarity", choices=("clear", "opaque"),
                        default="clear", help="掩模极性（默认 clear）")
    parser.add_argument("--core-nm", type=_decimal_argument, default=Decimal(1024),
                        help="tile 核心边长 nm（默认 1024）")
    parser.add_argument("--context-nm", type=_decimal_argument, default=Decimal(512),
                        help="tile 上下文边长 nm（默认 512）")
    parser.add_argument("--pixel-nm", type=_decimal_argument, default=Decimal(8),
                        help="光刻像素 nm（默认 8）")
    ...
    parser.add_argument("--out", type=Path, default=None,
                        help="留档目录（默认 output/lithography/<GDS 主干名>/，仓库根锚定）")
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = (args.out if args.out is not None else
                  _REPO_ROOT / "output" / "lithography" / args.gds.stem)
    with LayoutDB.open(args.gds, args.top) as database:  # 文件/顶层歧义在此报错
        ...
        if bounds is None:  # 空层无法出图（与迭代管线 prepare 阶段同口径）
            raise ValueError(
                f"目标层 {args.layer.layer}/{args.layer.datatype} 不含任何图形")
    ...
    manifest = save_lithography_pngs(
        args.gds, args.layer, MaskPolarity(args.polarity),
        core_dbu, context_dbu, pixel_dbu, model.config.canvas,
        model, args.batch, output_dir)
