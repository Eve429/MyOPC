...
## 50nm 定尺寸测试掩膜组交付事实（2026-08-21）

- 用户三项裁定（AskUserQuestion）：每组综合采样 + 调间距多档
  （dense/mid/loose）；文件夹命名 `p50_<边长nm>`；图形内缩 64nm 边框
  空区（联动 field/环带特性）。
- 与场景系本质差异：负板补区基准 = 固定设计区框 (0,0)-(s,s)（非图形
  bbox）→ 负板恒带 64nm 铬环框、包络恒整版；正板包络 = 图形区
  (64,64)-(s−64,s−64)——64 是 4/8/16nm pixel 公倍数，包络边恒整像素
  对齐。
- 尺寸考察点：1.024µm = 256px@4nm 单宏档；2.048µm = [2,2] 四宏档；
  组 2 左上象限逐位复刻组 1 同档位内容（跨尺寸对照：同内容、宏划分
  不同）。
- 实施教训：L/T/十字/U 用共边矩形拼装会触发"结构族互不接触"自检
  （merge 计数变化）——改整块单多边形构造；klayout `Layout.layers()`
  返回层数 int，遍历层索引用 `layer_indexes()`。
- 双板透光等价验证：p50_1024_mid 正负板同网格（field=整版）
  target_u8 **逐位相等**（真互补 + 1−(1−c)≡c 的端到端证明）；环带与
  context 扩张带两极性全 0。
- smoke：ILT 单宏（field_size 联动，环带警告文案正确）+ MB-OPC 四宏
