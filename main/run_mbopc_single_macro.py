"""单 macro、多 tile 的最简 MB-OPC 直接运行入口。"""
...
from main._mbopc_workflow import run_single_macro  # 共享工作流


def main() -> int:
    """读取唯一位置参数 config，运行单 macro 流程并打印中文摘要。"""
    if len(sys.argv) != 2:  # 参数数量不符
        print("用法：python main/run_mbopc_single_macro.py <config.toml>",
              file=sys.stderr)  # 提示
        return 2  # 参数错误退出码
    summary = run_single_macro(sys.argv[1])  # 恰好一个 macro 的完整流程
    print("单 macro simple MB-OPC 执行完成：")  # 摘要标题
