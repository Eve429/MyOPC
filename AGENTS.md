...
- 大 reticle 的普通 OPC 轮次固定参考边段、稳定 key、法向、拓扑顺序和 owner，不重新提边或切分；迭代只更新相对固定参考边界的全局绝对位移。只有显式 remesh 才能改变分段，并必须同步重建 key、归属和优化器状态。
...
- 更新合并应预分配 `d_next` 并按 owner segment index 直接 scatter，使用 epoch/bitset 检测重复写入；不得为每轮更新拼接全部 stable key 后执行全局排序。stable key 主要用于跨进程校验、checkpoint、恢复和诊断。
