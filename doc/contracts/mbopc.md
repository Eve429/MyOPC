...
- **公共趋势图**：`common.metric_trends.save_metric_trends` 接收结果序列到
  metrics JSON 的映射和字段列表，不依赖 MB-OPC。Simple 与 Gradient 在各自
  入口调用，输出 `metrics_trends/series_<id>.png`、总览图和
  `metric_trends.series_pngs/fields`；关闭开关时该字段为 null。ILT 后续可
  直接复用该接口，当前 ILT runner 不自动生成趋势图。
