...
def test_materialization_applies_reference_transforms(tmp_path: Path) -> None:
    ...
    layer = LayerSpec(1, 0)
    with LayoutDB.open(source) as db:
        batch = db.query([layer], db.bbox()).materialize()
        assert batch.stats is None
        assert batch.counts()[layer] == 5
        assert batch.region(layer).bbox() == kdb.Box(0, 0, 1000, 60)


def test_roi_query_does_not_return_distant_shapes(tmp_path: Path) -> None:
    """小 ROI 使用 KLayout 感知层级的空间限制，不返回远处图形。"""
    source = tmp_path / "distant.gds"
    native = kdb.Layout()
    index = native.layer(kdb.LayerInfo(1, 0))
    top = native.create_cell("TOP")
    top.shapes(index).insert(kdb.Box(0, 0, 100, 100))
    top.shapes(index).insert(kdb.Box(1000, 0, 1100, 100))
    native.write(str(source))
    layer = LayerSpec(1, 0)
    with LayoutDB.open(source) as db:
        left = db.query([layer], DbuBox(-10, -10, 110, 110)).materialize()
        right = db.query([layer], DbuBox(990, -10, 1110, 110)).materialize()
