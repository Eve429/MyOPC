"""Simple ILT 直接运行入口：读取 TOML 配置执行完整像素优化流程。"""
...
if str(_REPO_ROOT) not in sys.path:  # 避免重复插入
    sys.path.insert(0, str(_REPO_ROOT))  # 使 main/_simple_ilt_workflow 可导入

from main._simple_ilt_workflow import run_simple_ilt  # Simple 工作流


def main() -> int:
    """读取可选位置参数 config（默认仓库内 config/simple_ilt.toml）并执行。"""
    if len(sys.argv) > 2:  # 参数数量不符
        print("用法：python main/run_simple_ilt.py [config.toml]",
              file=sys.stderr)
        return 2  # 参数错误退出码
    config = (sys.argv[1] if len(sys.argv) == 2
              else str(_REPO_ROOT / "config" / "simple_ilt.toml"))
