...
class TestSingleMacroRunner:
    ...
    def test_source_lithography_archive_written(self, single):
        ...
        tmp, summary = single
        out_dir = tmp / "work" / "final_lithography_source"
        manifest = json.loads((out_dir / "manifest.json").read_text(encoding="utf-8"))
        assert manifest["tile_count"] > 0
        for tile in manifest["tiles"]:
            assert (out_dir / tile["nominal_png"]).is_file()
            assert (out_dir / tile["binary_png"]).is_file()
    ...
    def test_multi_top_cell_source_archive_completes(self, tmp_path):
        ...
        unused = layout.create_cell("UNUSED")
        unused.shapes(layout.layer(1, 0)).insert(kdb.Box(200, 10, 260, 80))
        gds = tmp_path / "dual_top.gds"
        layout.write(str(gds))
        # 不指名顶层时 LayoutDB.open 对多顶层报 AmbiguousTopCellError——
        # 源留档必须带 plan 顶层名打开，全流程才能走到收尾
        summary = workflow.run_mbopc(_write_config(tmp_path, gds))
        assert summary["source_lithography_tiles"] > 0
        assert (tmp_path / "work" / "final_lithography_source" / "manifest.json").is_file()
