...
def _pixel_size_dbu(dbu_um: float, pixel_size_nm: float) -> int:
    """把物理像素尺寸精确换算为整数数据库单位。"""
    if not isinstance(dbu_um, Real) or isinstance(dbu_um, bool) or not math.isfinite(float(dbu_um)) or dbu_um <= 0:
        raise RasterizationError("数据库单位必须是正有限数")
    if (
        not isinstance(pixel_size_nm, Real)
        or isinstance(pixel_size_nm, bool)
        or not math.isfinite(float(pixel_size_nm))
        or pixel_size_nm <= 0
    ):
    ...
    if rounded < 1 or not math.isclose(exact, rounded, rel_tol=0.0, abs_tol=1e-9):
        raise RasterizationError(f"{pixel_size_nm:g} nm 不能精确表示为当前版图的整数 DBU 像素尺寸")
    return rounded


def _image_shape(box: DbuBox, pixel_dbu: int, max_pixels: int) -> tuple[int, int]:
    """计算向右上补齐的图片尺寸，并在分配前执行内存保护。"""
    if not isinstance(max_pixels, Integral) or isinstance(max_pixels, bool) or max_pixels <= 0:
    ...
    if count > int(max_pixels):
        raise RasterizationError(f"请求图片为 {width}x{height}={count} 像素，超过上限 {int(max_pixels)}")
    return height, width
    ...
    *,
    ...
    for y0 in range(0, height, tile_rows):
        ...
        for x0 in range(0, width, tile_width):
            ...
            areas = np.asarray(clipped.rasterize(origin, kdb.Vector(pixel_dbu, pixel_dbu), columns, rows), dtype=dtype)
...
def _rasterize(region: kdb.Region, box: DbuBox, pixel_dbu: int, shape: tuple[int, int]) -> NDArray[np.uint8]:
...
def _save_png(pixels: NDArray[np.uint8], output_path: str | Path) -> Path:
    ...
    handle, temporary_name = tempfile.mkstemp(prefix=f".{output.stem}-", suffix=".png", dir=output.parent)
    os.close(handle)
...
def render_layout_region(
    ...
    *,
    ...
    return render_region_batch(
        batch, layer, database.dbu_um, pixel_size_nm, output_path=output_path, show=show, max_pixels=max_pixels
    )
