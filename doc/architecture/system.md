...
| `opc/iteration/ilt/` | ILT 求解器（Simple 像素优化 + 共享 record/result/loss） | 光刻实现、合并、进度库 |
| `main/` | 应用编排：共享宏管线生命周期（`_macro_pipeline.py`）、MB-OPC 公共工作流与适配器（`_mbopc_workflow.py` + `_simple/_gradient_mbopc_workflow.py`）、像素 ILT 公共工作流与适配器（`_ilt_workflow.py` + `_simple_ilt_workflow.py`）、各直接运行入口 | 领域算法 |
...
- **方法层**：`opc/iteration/` 下每个方法子包自足（各自的 config/state/
  solve），不建注册器；ILT 已有 simple 实现；diffopc 目录尚未创建。
