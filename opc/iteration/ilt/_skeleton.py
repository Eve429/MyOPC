...
def pack_batches(problem: PixelMacroProblem,
                 batch_size: int) -> list[BatchPack]:
    ...
    for batch_start in range(0, core_count, batch_size):
        ...
        packs.append(BatchPack(
            count=len(core_indices), targets_u8=targets,
            ownerships=ownerships, trainables=trainables, valids=valids,
            trainable_flat=trainable_flat, owned=owned, safe=safe))
    return packs


def check_common_entry(problem: PixelMacroProblem, model: LithographyModel, *,
    ...
    if ((require_context_pixel or curvature_weight > 0.0)
            and int(problem.macro.context_dbu) < int(problem.macro.pixel_dbu)):
        raise ValueError(
            f"{reason}要求 context 不小于 1 像素"
...
def run_state_batches(
    ...
    for pack in packs:
        target_tensor = torch.from_numpy(pack.targets_u8).to(
            device=device, dtype=torch.float32).div_(255.0)
        ownership_tensor = torch.from_numpy(pack.ownerships).to(device=device)
        index3 = torch.from_numpy(pack.trainables).to(device=device).view(
        ...
        if context_mode == "soft":
            context_source = torch.sigmoid(
                context_beta * (target_tensor * 2.0 - 1.0)).detach()
        else:
            context_source = (target_tensor >= 0.5).to(torch.float32)
        context = torch.where(
            valid_tensor, context_source, torch.zeros_like(context_source))
        ...
        curvature_value = (curvature_loss(
            mask if curvature_source == "mask" else printed["nominal"],
            ownership_tensor) if use_curvature else 0.0)
...
def total_loss_of(sums: dict[str, float], *, weight_process_l2: float,
                  weight_pvband: float, curvature_weight: float) -> float:
    """把 run_state_batches 的四项和聚合为加权总损失（张量同式）。"""
    return float(weighted_macro_loss(
        sums["nominal"], sums["process"], sums["pvband"], sums["curvature"],
        weight_process_l2=weight_process_l2, weight_pvband=weight_pvband,
        curvature_weight=curvature_weight))
