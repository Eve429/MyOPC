...
class ContourBatch:
    ...
    def __post_init__(self) -> None:
        ...
        if polygon_offsets.ndim != 1 or len(polygon_offsets) == 0 or polygon_offsets[0] != 0:
        ...
        if polygon_offsets[-1] != ring_count or (len(polygon_offsets) > 1 and np.any(np.diff(polygon_offsets) < 1)):
...
def extract_contours(batch: RegionBatch) -> Mapping[LayerSpec, ContourBatch]:
    ...
    return MappingProxyType({layer: extract_contour(region) for layer, region in batch.regions.items()})
