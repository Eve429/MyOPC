...
def _problem(region, macro=None, polarity="clear"):
    ...
    return prepare_pixel_macro_problem(batch, LAYER, polarity, macro, planning_bounds=BOUNDS, data_bounds=BOUNDS)
...
class _ConstPerCallModel:
    ...
    def forward_many(self, mask, conditions):
        ...
        self.calls += 1
        # 同上：零梯度依赖保证常数输出可反向
        return {c.name: mask * 0.0 + float(value) for c in conditions}
...
class _LocalAverageModel(_DoseModel):
    ...
    def forward_many(self, mask, conditions):
        ...
        self.calls += 1
        pooled = functional.avg_pool2d(mask[:, None], 3, stride=1, padding=1)[:, 0]
        return {c.name: pooled * self._conditions[c.name].dose for c in conditions}
...
def _float64_reference(problem, config, model):
    ...
    t_own = problem.target_u8[mrow0 : mrow0 + hm, mcol0 : mcol0 + wm].astype(np.float64) / 255.0
    core_count = problem.macro.core_count
    canvases = []
    for core_index in range(core_count):
        target = torch.from_numpy(problem.target_canvas(core_index)).to(torch.float64) / 255.0
        ownership = torch.from_numpy(problem.ownership_canvas(core_index))
        trainable = torch.from_numpy(problem.trainable_index_canvas(core_index))
    ...
    for state_index in range(config.iterations + 1):
        ...
        for target, ownership, trainable, valid in canvases:
            ...
            context = torch.where(valid, context_soft, torch.zeros_like(context_soft))
            mask = torch.where(trainable >= 0, soft, context)
            printed = model.forward_many(
                mask, (model.condition("nominal"), model.condition("dose_max"), model.condition("defocus_min"))
            )
            l_nom, l_proc, l_pv = owned_continuous_losses(
                printed["nominal"], printed["dose_max"], printed["defocus_min"], target, ownership
            )
            total = total + weighted(l_nom, l_proc, l_pv, config)
...
def weighted(l_nom, l_proc, l_pv, config):
    ...
    return l_nom + config.weight_process_l2 * l_proc + config.weight_pvband * l_pv
...
class TestParameterizationAndLoss:
    ...
    def test_hand_computed_losses_constant_model(self):
        ...
        assert record.total_loss == pytest.approx(expected_nom + 1.0 * expected_proc, rel=1e-5)
    ...
    def test_curvature_matches_numpy_reference(self):
        ...
        model = _ConstantModel(0.5)
        result = optimize_simple_macro(problem, model, config)
        kernel = np.array([[-1, 5, -1], [5, -16, 5], [-1, 5, -1]], dtype=np.float64) / 16.0
        expected = 0.0
        for core_index in range(problem.macro.core_count):
            target = problem.target_canvas(core_index).astype(np.float64) / 255.0  # [256,256]
            ...
            for dy in range(3):
                for dx in range(3):
                    convo += kernel[dy, dx] * soft[dy : dy + 254, dx : dx + 254]
            expected += float(((convo**2) * own[1:-1, 1:-1]).sum())
        assert result.records[0].curvature_loss == pytest.approx(expected, rel=1e-3, abs=1e-6)
        assert result.records[0].total_loss == pytest.approx(
            result.records[0].nominal_l2 + result.records[0].process_l2 + 2.0 * expected, rel=1e-3, abs=1e-6
        )
    ...
    def test_owned_losses_count_ownership_only(self):
        ...
        l_nom, l_proc, l_pv = owned_continuous_losses(printed_nom, printed_max, printed_min, target, ownership)
...
class TestStateAndBarrier:
    ...
    def test_single_core_matches_float64_reference(self):
        ...
        for record, (total, parts) in zip(result.records, reference["losses"]):
            ...
            assert record.nominal_l2 == pytest.approx(parts["nominal"], rel=1e-4, abs=1e-9)
            assert record.process_l2 == pytest.approx(parts["process"], rel=1e-4, abs=1e-9)
        assert result.best_state_index == reference["best_state"]
        assert np.allclose(result.best_parameters, reference["best"], rtol=1e-4, atol=1e-6)
...
class TestMacroBestAndBatch:
    ...
    def test_best_is_macro_total_not_per_core_patchwork(self):
        ...
        totals = [
            sum(core_loss(core, values[state * core_count + core]) for core in range(core_count)) for state in range(3)
        ]
        assert result.best_state_index == int(np.argmin(totals))
        # 判别性：左列材料核与右列空核的局部最优必须不同，且宏最优
        # 与材料核局部最优不同（拼贴/局部最优都会选错）
        per_core_best = [
            int(np.argmin([core_loss(core, values[s * core_count + core]) for s in range(3)]))
    ...
    def test_batch_size_invariance(self):
        ...
        for batch_size in (1, 2, 4):
            results.append(optimize_simple_macro(problem, model, _config(iterations=2, batch_size=batch_size)))
        base = results[0]
        for other in results[1:]:
            assert other.best_state_index == base.best_state_index
            assert np.allclose(other.best_parameters, base.best_parameters, rtol=1e-5, atol=1e-6)
            for record_b, record_o in zip(base.records, other.records):
                assert record_o.total_loss == pytest.approx(record_b.total_loss, rel=1e-5)
...
class TestCrossCoreGradient:
    ...
    def test_shared_pixel_receives_summed_gradient(self, monkeypatch):
        ...
        for batch_size in (1, 2):
            ...
            results[batch_size] = optimize_simple_macro(problem, model, _config(iterations=1, batch_size=batch_size))
    ...
    def test_parameters_only_on_macro_ownership(self):
        ...
        assert np.logical_and(result.soft_mask >= 0.0, result.soft_mask <= 1.0).all()
...
class TestMacroSeamConsistency:
    ...
    def _state0_raster(problem):
        ...
        optimize_simple_macro(problem, model, _config(iterations=1, batch_size=problem.macro.core_count))
        ...
        for core_index in range(problem.macro.core_count):
            ...
            grid[r0:r1, c0:c1] = canvas[low_y : low_y + (r1 - r0), low_x : low_x + (c1 - c0)]
    ...
    def test_context_matches_neighbor_state0(self):
        ...
        macros = plan_macros_local(
            DbuBox(0, 0, 160, 80), macro_grid=(2, 1), core_size_dbu=40, context_dbu=20, pixel_dbu=4, canvas_pixels=256
        )
        problems = []
        for macro in macros:  # 相邻两宏各自独立 problem（独立 macro 语义）
            batch = RegionBatch({LAYER: kdb.Region(kdb.Box(20, 20, 140, 60))}, macro.query_box)
            problems.append(
        ...
        target = problems[1].target_u8[b_r0 : b_r0 + rows, b_c0 : b_c0 + cols].astype(np.float64) / 255.0
...
class TestRealModel:
    ...
    def test_real_cpu_update_finite_and_effective(self, cpu_model):
        ...
        result = optimize_simple_macro(problem, cpu_model, _config(iterations=1, batch_size=1))
    ...
    def test_real_cpu_updates_aligned_geometry(self, cpu_model):
        ...
        initial = 2.0 * problem.target_u8[r0 : r0 + hm, c0 : c0 + wm].astype(np.float32) / 255.0 - 1.0
    ...
    def test_real_cuda_matches_cpu(self, cpu_model):
        ...
        cpu_result = optimize_simple_macro(problem, cpu_model, _config(iterations=1, batch_size=1))
        cuda_result = optimize_simple_macro(problem, cuda_model, _config(iterations=1, batch_size=1))
...
class TestCallCountsAndProgress:
    ...
    def test_mixed_batch_progress_counts(self):
        ...
        received = []
        model = _DoseModel()
        optimize_simple_macro(problem, model, _config(iterations=2, batch_size=6), on_tiles_completed=received.append)
    ...
    def test_no_conv_when_curvature_zero(self, monkeypatch):
        ...
        monkeypatch.undo()
        optimize_simple_macro(problem, _DoseModel(), _config(curvature_weight=1.0))  # 正常执行
    ...
    def test_curvature_requires_at_least_one_context_pixel(self):
        ...
        with pytest.raises(ValueError, match="context"):
            optimize_simple_macro(problem, _DoseModel(), _config(curvature_weight=1.0))
...
class TestFinalContextHelper:
    ...
    def test_helper_equals_solver_context_formula(self):
        ...
        config = _config()
        for core_index in range(problem.macro.core_count):
            actual = build_simple_final_context_canvas(problem, core_index, config)
            valid = problem.context_valid_canvas(core_index)
            target = problem.target_canvas(core_index)
            # numpy 同式复算：helper 与 _soft0 是同一实现路径，必须逐位相等
            expected_np = np.where(valid, _soft0(target.astype(np.float32) / 255.0), 0.0)
            ...
            expected_t = torch.sigmoid(4.0 * (target_t * 2.0 - 1.0)).numpy()
            np.testing.assert_allclose(actual, np.where(valid, expected_t, 0.0), rtol=0.0, atol=1e-6)
