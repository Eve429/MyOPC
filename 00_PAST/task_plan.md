...
## 阶段 78：开发手册常见陷阱与 FAQ 审查

| 阶段 | 状态 | 内容 |
|---|---|---|
| 78 | 已完成 | 对照当前代码逐条区分真实 bug、必要数据契约、保护性异常和运维规则；形成 raster 方向、MB-OPC 迭代次数、Region 生命周期和拓扑回滚的最小修复优先级，本轮未修改生产代码 |

## 阶段 79–81：FAQ 契约修复

| 阶段 | 状态 | 内容 |
|---|---|---|
| 79 | 已完成 | 经用户授权统一 Geometry/OPC raster 返回数组方向，把翻转收口到 PNG/查看器/诊断输出边界，并配置 Ruff 排除用户 notebook |
| 80 | 已完成 | 修复 simple MB-OPC 末次更新丢失，明确 `iterations` 为更新次数并保证最终候选经过拓扑校验和光刻评价 |
| 81 | 已完成 | 补齐生命周期/方向/迭代回归；210 项全量测试和严格性能基准通过，中文 docstring/重复实现/文档/差异审计无遗漏，专项报告已同步并创建本地提交 |

## 阶段 82–87：版图极性、GLP 与运行配置

| 阶段 | 状态 | 内容 |
|---|---|---|
| 82 | 已完成 | 固化干净基线，复核版图读取、预检、raster、问题准备和全部运行入口的真实调用点 |
| 83 | 已完成 | 在 `layout/` 内最小实现严格 GLP 读取，并让预检复用已打开的 `LayoutDB`，避免源文件重复解析 |
| 84 | 已完成 | 增加显式 `clear/opaque` 极性，保证内部光学数组始终以 1 表示透光且输出恢复源图形语义 |
| 85 | 已完成 | 增加六份 TOML 默认配置及严格合并器，为全部 `main/run_*.py` 和离线入口接入 `--config` |
...
- `opaque` 极性必须显式给出处理框，内部 raster 仅在处理框内执行透光反相，不能把处理框边界物化成可移动 OPC 边。
- 动态 SRAF 本轮只形成可实施设计，不建立尚无调用方的接口、注册器或占位目录。
- TOML 只负责命令行默认值和来源记录；Python 算法 API 继续由现有 dataclass 与函数参数校验，不引入运行期全局配置对象。

## 本轮新增错误记录

| 错误 | 次数 | 处理 |
|---|---:|---|
| 初次只读探查猜测了不存在的 `opc/input/physical.py` | 1 | 以 `rg --files` 为唯一文件清单，真实实现位于 `opc/input/mask.py` |
| 一次内联 Python 探查脚本因 PowerShell 换行转义产生语法错误 | 1 | 未修改文件；后续将探查拆为原生命令或 here-string，并保留失败记录 |
| 一次解析改造删除了归档 metadata 仍使用的 `selected_top` | 1 | 专项测试通过但 Ruff 立即捕获；恢复两个报告局部变量，不增加兼容分支 |
| 旧 raster 归档极性兼容校验把结果赋给未使用局部变量 | 1 | 保留 Enum 构造的校验副作用，删除无用途名称并复跑静态检查 |
| 多入口配置大补丁因 `run_simpleilt` 签名换行与预期上下文不同而未应用 | 1 | 原子失败无文件变化；拆为精确短补丁后逐入口 compile/Ruff |
| `run_ilt` 改用配置解析器后遗留未使用 `argparse` 导入 | 1 | Ruff 立即捕获并删除，不保留旧解析器依赖 |
| 配置解析器类型被 argparse 默认继承给离线子解析器，缺少 workflow 参数 | 1 | 根解析器仍统一读配置，子解析器显式使用标准 `ArgumentParser`，避免重复读取与构造参数 |
