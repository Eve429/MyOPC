...
class _EdgeGradientMask(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        g_mid = v00 * (1.0 - wx) * (1.0 - wy) + v01 * wx * (1.0 - wy) + v10 * (1.0 - wx) * wy + v11 * wx * wy
...
def _profile_d_s(error: torch.Tensor, slots: torch.Tensor, xy: torch.Tensor) -> torch.Tensor:
    ...
    sample = v00 * (1.0 - wx) * (1.0 - wy) + v01 * wx * (1.0 - wy) + v10 * (1.0 - wx) * wy + v11 * wx * wy
...
def _prepare_gradient_context(
    ...
    for core_index in range(core_count):
        ...
        if epe_enabled and len(owner_members):
            ...
            profile_dbu = midpoints[:, None, :] + epe_offsets[None, :, None] * normals[:, None, :]
...
def _evaluate_state(
    ...
    *,
    ...
    for core_indices, targets, masks, ownership in iter_core_batches(
        problem, ctx.pack, current_region, target_cache, batch_size=config.batch_size, rasterize=rasterize_mask_canvas
    ):
        ...
        for slot, core_index in enumerate(core_indices):  # 梯度采样收集
            ...
            if len(sampling_members):  # 梯度采样：全部 membership 中 owner 段
                ...
                # DBU→canvas 唯一换算
                member_mids.append(
                    points_to_canvas(midpoints_dbu, spec.context_box, ctx.pack.pixel_dbu, ctx.pack.canvas_pixels)
                )
                member_slots.append(np.full(len(sampling_members), slot, dtype=np.int64))
                member_params.append(ctx.segment_to_parameter[sampling_members].astype(np.int64))
            ...
            if len(owner_members) and ctx.epe_length_sum > 0.0:
                ...
                epe_slots.append(np.full(len(owner_members), slot, dtype=np.int64))
        ...
        target_tensor, hard, ownership_tensor = upload_eval_batch(targets, masks, ownership, ctx.device)
        if build_graph:  # STE：forward 数值不变，autograd 边接到位移
            ...
            mask_tensor = _EdgeGradientMask.apply(hard, local, slots, mids, ctx.pack.pixel_dbu)
        else:
        ...
        if epe_slots:
            epe_slot_tensor = torch.from_numpy(np.concatenate(epe_slots)).to(device=ctx.device)
            epe_xy_tensor = torch.from_numpy(np.concatenate(epe_xy_parts)).to(device=ctx.device, dtype=torch.float32)
            epe_len_tensor = torch.from_numpy(np.concatenate(epe_len_parts)).to(device=ctx.device, dtype=torch.float32)
        ...
        if build_graph:  # 建图版连续 loss：backward 累积到 parameters.grad
            ...
            l_proc = (((dose_max - target_tensor) ** 2 + (defocus_min - target_tensor) ** 2) * ownership_tensor).sum()
            l_pv = ((dose_max - defocus_min) ** 2 * ownership_tensor).sum()
            batch_loss = (
                config.weight_nominal_l2 * l_nom + config.weight_process_l2 * l_proc + config.weight_pvband * l_pv
            ) / ctx.pack.total_pixels
            if epe_slot_tensor is not None:
                # 第四项 loss 与三项共用同一次 backward；penalty 为
                # zero-based sigmoid，len 归一保证切段不变性（DEC-007）。
                d_s = _profile_d_s(nominal_error, epe_slot_tensor, epe_xy_tensor)
                penalty = 2.0 * (torch.sigmoid(config.epe_steepness * d_s) - 0.5)
                batch_epe = (epe_len_tensor * penalty).sum() / ctx.epe_length_sum
            ...
            with torch.no_grad():  # 纯评价路径不建图
                nominal_error = (nominal - target_tensor) ** 2
                triple = (
                    (nominal_error * ownership_tensor).sum(),
                    (((dose_max - target_tensor) ** 2 + (defocus_min - target_tensor) ** 2) * ownership_tensor).sum(),
                    ((dose_max - defocus_min) ** 2 * ownership_tensor).sum(),
                )
                if epe_slot_tensor is not None:
                    d_s = _profile_d_s(nominal_error, epe_slot_tensor, epe_xy_tensor)
                    penalty = 2.0 * (torch.sigmoid(config.epe_steepness * d_s) - 0.5)
        ...
        with torch.no_grad():  # 离散诊断只读数值，不进入训练
            ...
            # （evaluate_* 补丁锚在本模块）
            probe_slots_np, inner_np, outer_np = assemble_probe_batch(ctx.pack, core_indices)
            ...
            if epe_result is not None:
                ...
                diag["ambiguous"] += int(epe_result.ambiguous.cpu().numpy().sum())
    ...
    total_loss = (
        config.weight_nominal_l2 * nominal_loss
    ...
    return _GradientStateEvaluation(
        ...
        pvband_loss=pvband_loss,
        l2=diag["l2"],
        pvband=diag["pvband"],
        epe=diag["epe"],
        valid_probes=diag["valid"],
...
def optimize_gradient_macro(
    ...
    *,
    ...
    if len(owner_ids) == 0:
        ...
        return GradientMBOPCResult(np.zeros(segment_count, dtype=np.float64), (empty,), 0, "no_owned_segments", None)
    # 静态输入只准备一次，全部状态迭代复用。
    ...
    for state_index in range(config.iterations + 1):
        ...
        evaluation = _evaluate_state(
            ...
            parameters,
            current_region,
            current_segment_midpoints,
            build_gradient=can_update,
            on_tiles_completed=on_tiles_completed,
        )
        if not (
            isfinite(evaluation.total_loss)
            ...
            raise FloatingPointError(f"{ctx.pack.macro_id} state {state_index} 连续 loss 非有限")
        records.append(
            GradientMBOPCIterationRecord(
                ...
                pvband_loss=evaluation.pvband_loss,
                l2=evaluation.l2,
                pvband=evaluation.pvband,
                epe=evaluation.epe,
