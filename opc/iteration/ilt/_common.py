...
def owned_continuous_losses(
        nominal: torch.Tensor, dose_max: torch.Tensor,
        defocus_min: torch.Tensor, target: torch.Tensor,
        ownership: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    process_l2 = (((dose_max - target) ** 2
                   + (defocus_min - target) ** 2) * ownership).sum()
    pvband_loss = ((dose_max - defocus_min) ** 2 * ownership).sum()
    return nominal_l2, process_l2, pvband_loss


def curvature_loss(mask: torch.Tensor,
                   ownership: torch.Tensor) -> torch.Tensor:
    ...
    return (curvature.square()
            * ownership[:, 1:-1, 1:-1]).sum()


def weighted_macro_loss(
        nominal_l2: torch.Tensor, process_l2: torch.Tensor,
        pvband_loss: torch.Tensor,
        curvature_value: torch.Tensor | float,
        *, weight_process_l2: float, weight_pvband: float,
        curvature_weight: float) -> torch.Tensor:
