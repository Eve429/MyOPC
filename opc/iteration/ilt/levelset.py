...
class LevelSetILTConfig:
    ...
    def __post_init__(self) -> None:
        ...
        values = (self.step_size, self.weight_process_l2, self.weight_pvband, self.curvature_weight)
...
def signed_distance_initialization(target_u8: np.ndarray, pixel_nm: float = 1.0) -> np.ndarray:
    ...
    if bool(binary.all()):
        return np.full((height, width), -float(max(height, width)) * spacing, np.float32)
    if not bool(binary.any()):
        return np.full((height, width), float(max(height, width)) * spacing, np.float32)
    # 顺序执行 outside/inside EDT 并即时释放 float64 中间量：初始化峰值
    # 不超过一份 float64 距离场 + 一份 float32 结果（REQ-003 SHOULD）。
    phi = distance_transform_edt(~binary, sampling=(spacing, spacing)).astype(np.float32)
...
def macro_gradient_magnitude(
    problem: PixelMacroProblem, initial_query_phi: np.ndarray, macro_phi: np.ndarray, pixel_nm: float = 1.0
) -> np.ndarray:
    ...
    if np.asarray(initial_query_phi).shape != problem.query_shape or np.asarray(macro_phi).shape != (hm, wm):
    ...
    halo = np.array(
        initial_query_phi[mrow0 - 1 : mrow0 + hm + 1, mcol0 - 1 : mcol0 + wm + 1], dtype=np.float32, copy=True
    )
...
class _LevelSetBinarize(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx: torch.autograd.function.FunctionCtx, levelset: torch.Tensor, grad_magnitude: torch.Tensor | None
    ) -> torch.Tensor:
...
def build_levelset_final_context_canvas(
    problem: PixelMacroProblem, core_index: int, config: LevelSetILTConfig
) -> np.ndarray:
    ...
    return np.where(problem.context_valid_canvas(core_index), hard, 0.0).astype(np.float32)
...
def optimize_levelset_macro(
    ...
    *,
    ...
    check_common_entry(problem, model, require_context_pixel=True, reason="LevelSet ")
    pixel_dbu = int(problem.macro.pixel_dbu)
    ...
    initial_query_phi = signed_distance_initialization(problem.target_u8, pixel_nm=pixel_nm)
    if not np.isfinite(initial_query_phi).all():
        raise FloatingPointError(f"{problem.macro.macro_id} 初始 SDF 非有限")
    macro_phi = torch.from_numpy(initial_query_phi[mrow0 : mrow0 + hm, mcol0 : mcol0 + wm].copy())
    optimizer = torch.optim.Adam(
        [macro_phi], lr=config.step_size, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0, amsgrad=False
    )  # 超参与 REQ-008 契约锁定
    ...
    for state_index in range(config.iterations + 1):
        ...
        if build_gradient:
            grad_magnitude = macro_gradient_magnitude(
                problem, initial_query_phi, macro_phi.numpy(), pixel_nm=pixel_nm
            ).reshape(-1)
        ...
        def slot_values(
            ...
            local = torch.from_numpy(_phi[pack.safe]).to(device=device, dtype=torch.float32)
            if _build:
                local.requires_grad_(True)
            local_grad = (
                torch.from_numpy(_coefficient[pack.safe]).to(device=device, dtype=torch.float32) if _build else None
            )
            values = _LevelSetBinarize.apply(
                local.view(pack.count, canvas_pixels, canvas_pixels),
                None if local_grad is None else local_grad.view(pack.count, canvas_pixels, canvas_pixels),
            )

            def collect(finished: BatchPack, _local=local, _macro_gradient=_macro_gradient) -> None:
                """scatter-add raw sum：同一像素出现在多个 core context 时
                梯度相加，绝不按出现次数平均（REQ-007）。"""
                grad_np = _local.grad.detach().cpu().numpy()
                np.add.at(_macro_gradient, finished.trainable_flat[finished.owned], grad_np[finished.owned])

            return SlotForward(values=values, collect_gradient=collect if _build else None)
        ...
        if not np.isfinite(macro_gradient).all():
            raise FloatingPointError(f"{problem.macro.macro_id} state {state_index} 宏梯度非有限")
        macro_phi.grad = torch.from_numpy(macro_gradient.reshape(hm, wm))  # 唯一宏梯度（raw sum 结果）
        optimizer.step()
        optimizer.zero_grad(set_to_none=True)
        if not bool(torch.isfinite(macro_phi).all()):
            raise FloatingPointError(f"{problem.macro.macro_id} state {state_index} 更新后 phi 非有限")
        for adam_state in optimizer.state.values():  # m/v 常驻 CPU
            for value in adam_state.values():
                if isinstance(value, torch.Tensor) and not bool(torch.isfinite(value).all()):
                    raise FloatingPointError(f"{problem.macro.macro_id} state {state_index} Adam 状态非有限")
