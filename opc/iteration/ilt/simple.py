    ...
    *,
    ...
    for state_index in range(config.iterations + 1):
        ...
        def slot_values(
            ...
            local = torch.from_numpy(local_values).to(device=device, dtype=torch.float32)
            if _build:
                local.requires_grad_(True)
            values = torch.sigmoid(beta * local.view(pack.count, canvas_pixels, canvas_pixels))

            def collect(finished: BatchPack, _local=local, _gradient=_gradient) -> None:
                """scatter-add 求和：同一像素出现在多个 core context 时
                梯度相加，绝不按出现次数平均。"""
                grad_np = _local.grad.detach().cpu().numpy()
                np.add.at(_gradient, finished.trainable_flat[finished.owned], grad_np[finished.owned])
        ...
        if not np.isfinite(flat_parameters).all():
            raise FloatingPointError(f"{problem.macro.macro_id} state {state_index} 更新后参数非有限")
    # best 物化：软掩膜/二值都在 CPU float32 域完成，不携带 autograd
    best_parameters = best_flat.reshape(hm, wm)
    soft_mask = torch.sigmoid(beta * torch.from_numpy(best_parameters)).numpy()
...
def build_simple_final_context_canvas(
    problem: PixelMacroProblem, core_index: int, config: SimpleILTConfig
) -> np.ndarray:
