...
def _selected_pixels(reference: torch.Tensor, ownership_mask: torch.Tensor | None) -> torch.Tensor:
...
def evaluate_binary_l2(
    target: torch.Tensor, nominal: torch.Tensor, threshold: float = 0.5, ownership_mask: torch.Tensor | None = None
) -> int:
...
def evaluate_pvband(
    maximum: torch.Tensor, minimum: torch.Tensor, threshold: float = 0.5, ownership_mask: torch.Tensor | None = None
) -> int:
...
def evaluate_edge_probes(
    ...
    if (
        ...
        or inner.ndim != 2
        or inner.shape[1] != 2
        or outer.shape != inner.shape
        or len(batches) != len(inner)
    ):
