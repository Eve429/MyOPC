...
class CurvMultiConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (not isinstance(self.scales, tuple) or not self.scales
                or any(not isinstance(scale, int)
                       or isinstance(scale, bool) or scale <= 0
                       for scale in self.scales)):
            raise ValueError("scales 必须是非空正整数元组")
        if (self.scales[-1] != 1
                or any(left <= right for left, right
                       in zip(self.scales, self.scales[1:]))):
...
def optimize_curvmulti_macro(
    ...
    if any(hm % scale or wm % scale for scale in config.scales):
        raise ValueError(
            f"宏 ownership {wm}x{hm} 必须整除全部 scale {config.scales}")
    if min(hm // config.scales[0], wm // config.scales[0]) < kernel:
        raise ValueError(
            "最粗控制网格不能小于 smoothing_kernel"
            f"（{min(hm // config.scales[0], wm // config.scales[0])} < {kernel}）")
    ...
    for stage_index, scale in enumerate(config.scales):
        ...
        stage_initial = (resize_image(target_grid, control_shape, "area")
                         if previous_stage_best is None else
                         resize_image(previous_stage_best, control_shape,
                                      "nearest"))
        ...
        for stage_state in range(config.iterations_per_stage + 1):
            ...
            records.append(ILTStateRecord(
                state_index=state_index, stage_index=stage_index,
                stage_state_index=stage_state, scale=scale,
            ...
            if gradient is None or not torch.isfinite(gradient).all():
                raise FloatingPointError(
                    f"{problem.macro.macro_id} state {state_index - 1} 控制网格梯度非有限")
            optimizer.step()
            if not torch.isfinite(control).all():
                raise FloatingPointError(
                    f"{problem.macro.macro_id} state {state_index - 1} 更新后控制网格非有限")
    ...
    best_parameters = resize_image(
        best_control, (hm, wm), "nearest")[0].numpy()
    soft_mask = resize_image(
        smooth_sigmoid_mask(best_control, kernel, beta, offset),
        (hm, wm), "nearest")[0].numpy()
...
def build_curvmulti_final_context_canvas(
        ...
        config: CurvMultiConfig) -> np.ndarray:
