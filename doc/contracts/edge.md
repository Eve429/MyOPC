...
class SegmentBatch:                               # frozen；参考几何唯一数组真源
    ...
    segment_edge_ids: Int32Array[S]               # 段所属数学边起点索引
...
- **切线分裂**：当前 `prepare_macro_problem` 只在 macro ownership 外边界
  处分裂；core 内部切线不强制分裂。斜边交点参数由整数端点+整数切线计算，
  分裂碎片沿用原段数学边号；同一切线交点去重（零长碎段回归）。需要同时
  隔离 macro 与 core 的调用方可使用
  `opc.input.edge.problem._split_segments_at_macro_and_core_cuts` 的内部入口。
