...
def _macros():
    """返回 2×2 macro 网格。"""
    return plan_macros(BOUNDS, macro_grid=(2, 2), core_size_dbu=40, context_dbu=20, pixel_dbu=4, canvas_pixels=256)


def _problem(region, macro):
    """把原生 Region 直接包装为 RegionBatch 并生成 macro problem。"""
    batch = RegionBatch({LAYER: region}, macro.query_box)
    return prepare_macro_problem(batch, LAYER, "clear", CFG, macro, data_bounds=BOUNDS)
...
def _assert_problem_invariants(problem, source):
    ...
    assert np.all((problem.owner_indices >= -1) & (problem.owner_indices < problem.macro.core_count))
    ...
    for core_index in range(problem.macro.core_count):
        ...
        owned_here = {int(s) for s in np.flatnonzero(problem.owner_indices == core_index)}
...
class TestOpaqueSurround:
    ...
    def _opaque_problem(self, region, macro=None, data_bounds=BOUNDS):
        ...
        batch = RegionBatch({LAYER: region}, macro.query_box)
        return prepare_macro_problem(batch, LAYER, "opaque", CFG, macro, data_bounds=data_bounds)
    ...
    def test_complement_plate_boundary_edges_merge_away(self):
        ...
        chrome = kdb.Region(kdb.Box(0, 0, 80, 80)) - kdb.Region(kdb.Box(30, 30, 50, 50))
        problem = self._opaque_problem(chrome, data_bounds=DbuBox(0, 0, 80, 80))
        mids = self._midpoints(problem)
        on_bounds = (
            np.isclose(mids[:, 0], 0)
            | np.isclose(mids[:, 0], 80)
            | np.isclose(mids[:, 1], 0)
            | np.isclose(mids[:, 1], 80)
        )
    ...
    def test_clear_ignores_data_bounds(self):
        ...
        for bounds in (BOUNDS, macro.query_box, DbuBox(30, 30, 50, 50)):
            batch = RegionBatch({LAYER: region}, macro.query_box)
            problem = prepare_macro_problem(batch, LAYER, "clear", CFG, macro, data_bounds=bounds)
    ...
    def test_opaque_empty_region_far_from_data_fills_query(self):
        ...
        problem = prepare_macro_problem(
            batch, LAYER, "opaque", CFG, macro, data_bounds=DbuBox(200, 200, 240, 240)
        )  # 与查询不相交
        rebuilt = reconstruct_region(problem, np.zeros(problem.segments.segment_count))
        assert int((rebuilt ^ kdb.Region(macro.query_box.to_native())).area()) == 0
...
class TestSharedDiagonal:
    ...
    def test_shared_macro_boundary_split_points_are_identical(self):
        ...
        macros = _macros()
        region = kdb.Region(kdb.Polygon([kdb.Point(60, 100), kdb.Point(100, 100), kdb.Point(100, 20)]))
...
class TestGeometryMatrix:
    ...
    def test_concave_polygon_crossing_horizontal_macro_boundary(self):
        ...
        for macro in _macros():
            ...
            assert problem.segments.contours.ring_count == _reference_ring_count(region, macro)

    def test_two_dbu_narrow_ring_crossing_cuts(self):
        """2 DBU 宽窄环（外框−内孔差 2）跨 core 切线与 macro 边界。"""
        narrow = kdb.Region(kdb.Box(20, 70, 60, 110)) - kdb.Region(kdb.Box(22, 72, 58, 108))
        for macro in _macros():
    ...
    def test_slope_crossing_vertical_macro_boundary(self):
        ...
        region = kdb.Region(kdb.Polygon([kdb.Point(60, 20), kdb.Point(120, 20), kdb.Point(90, 50)]))
        ...
        assert np.array_equal(_edge_split_params(left, first, second), _edge_split_params(right, first, second))

    def test_steep_slope_crossing_horizontal_macro_boundary(self):
        """陡斜边（斜率 8/3）跨横向切线 y=80：上下两侧分裂参数逐位一致。"""
        region = kdb.Region(kdb.Polygon([kdb.Point(20, 60), kdb.Point(60, 60), kdb.Point(35, 100)]))
        ...
        assert np.array_equal(_edge_split_params(lower, first, second), _edge_split_params(upper, first, second))

    def test_slope_through_macro_corner_point(self):
        """斜边精确穿过 macro 角点 (80,80)：四个 macro 的分裂参数全部一致。"""
        region = kdb.Region(kdb.Polygon([kdb.Point(60, 60), kdb.Point(100, 60), kdb.Point(60, 100)]))
    ...
    def test_two_by_two_aref_spreads_across_macros(self, tmp_path):
        ...
        unit.shapes(layout.layer(1, 0)).insert(kdb.Box(60, 30, 100, 50))
        top = layout.create_cell("TOP")
        top.insert(kdb.CellInstArray(unit.cell_index(), kdb.Trans(0, 0), kdb.Vector(40, 0), kdb.Vector(0, 60), 2, 2))
...
class TestReconstructionMidpoints:
    ...
    def _midpoints_and_rigid(problem, displacements):
        ...
        rigid = (geometry.starts + geometry.ends) * 0.5 + geometry.normals * displacements[:, None]
        return midpoints, rigid, geometry

    @staticmethod
    def _edge_mask(geometry, axis, value):
        """选出参考几何中整条落在某坐标线上的段（如底边全部 fragment）。"""
        return (geometry.starts[:, axis] == value) & (geometry.ends[:, axis] == value)
    ...
    def test_zero_displacement_matches_fragment_midpoints(self):
        ...
        np.testing.assert_allclose(midpoints, (geometry.starts + geometry.ends) * 0.5, atol=1e-12)
    ...
    def test_corner_adjacent_midpoint_follows_miter_junction(self):
        ...
        midpoints, rigid, _ = self._midpoints_and_rigid(problem, displacements)
        # 底边上最靠近右下角的 fragment（corner 段，x 上界触 70）。
        corner_frag = int(
            np.flatnonzero(bottom_ids & (np.maximum(geometry.starts[:, 0], geometry.ends[:, 0]) == 70))[0]
        )
        ...
        split_x = float(np.minimum(geometry.starts[corner_frag, 0], geometry.ends[corner_frag, 0]))
        ...
        assert not np.allclose(midpoints[corner_frag], rigid[corner_frag], atol=1e-9)
        # 远离改动角的底边中段：两边界均为同位移内部切分，仍与刚体一致。
        middle = int(
            np.flatnonzero(
                bottom_ids
                & (np.minimum(geometry.starts[:, 0], geometry.ends[:, 0]) > 10)
                & (np.maximum(geometry.starts[:, 0], geometry.ends[:, 0]) < 70)
            )[0]
        )
        np.testing.assert_allclose(midpoints[middle], rigid[middle], atol=1e-12)
    ...
    def test_jog_boundary_uses_own_offset_ends(self):
        ...
        lo_x = np.minimum(geometry.starts[neighbours, 0], geometry.ends[neighbours, 0])
        ...
        split_x = float(np.minimum(geometry.starts[corner_frag, 0], geometry.ends[corner_frag, 0]))
        second_lo = float(np.minimum(geometry.starts[second, 0], geometry.ends[second, 0]))
        # corner 段实际区间 = [自身 offset 起点, 右角 junction]（右边未动，
        # 交点 = (70, 10 + 3·n_y)）。
        np.testing.assert_allclose(midpoints[corner_frag], [split_x * 0.5 + 35.0, 10.0 + n_bottom[1] * 3.0], atol=1e-9)
        # 紧邻中段两侧都是 jog 边界：两端均为自身 offset 端点（纯法向平移）。
        np.testing.assert_allclose(
            midpoints[second], [(second_lo + split_x) * 0.5, 10.0 + n_bottom[1] * -2.0], atol=1e-9
        )
    ...
    def test_midpoints_only_computed_when_requested(self):
        ...
        assert full.segment_midpoints.shape == (problem.segments.segment_count, 2)
        # 两种请求下的整数轮廓完全一致（中点是纯附加产物）。
        np.testing.assert_array_equal(plain.contours.vertices, full.contours.vertices)
        np.testing.assert_array_equal(plain.contours.ring_offsets, full.contours.ring_offsets)

    def test_bevel_boundary_uses_offset_ends(self):
        """miter 超限时 bevel：corner 邻段取自身 offset 端点而非交点。"""
        cfg = FragmentationConfig(
            corner_length_dbu=8.0, max_segment_length_dbu=16.0, max_displacement_dbu=10.0, miter_limit=1.0
        )  # 最小合法阈值强制 bevel
        macro = _macros()[0]
        ...
        np.testing.assert_allclose(
            midpoints[corner_frag], [(split_x + 70.0) * 0.5, 10.0 + n_bottom[1] * 3.0], atol=1e-9
        )
        assert midpoints.shape == (problem.segments.segment_count, 2)
        assert np.all(np.isfinite(midpoints))
...
class TestTopologyPreservation:
    ...
    def test_donut_rings_survive_ownership_split(self):
        ...
        donut = kdb.Region(kdb.Box(10, 10, 150, 150)) - kdb.Region(kdb.Box(60, 60, 100, 100))
    ...
    def test_zero_displacement_owner_coverage_merges_to_input(self):
        ...
        for macro in _macros():
            problem = _problem(region, macro)
            zeros = np.zeros(problem.segments.segment_count)
            merged += reconstruct_region(problem, zeros) & kdb.Region(macro.ownership_box.to_native())
    ...
    def test_array_references_materialize_into_problem(self, tmp_path):
        ...
        top = layout.create_cell("TOP")
        top.insert(kdb.CellInstArray(unit.cell_index(), kdb.Trans(10, 10), kdb.Vector(60, 0), kdb.Vector(0, 60), 3, 3))
        path = tmp_path / "aref.gds"
        layout.write(str(path))
        macro = _macros()[0]
        with LayoutDB.open(path) as database:
...
class TestPreparationValidation:
    ...
    def test_query_box_mismatch_rejected(self):
        ...
        batch = RegionBatch({LAYER: kdb.Region(kdb.Box(0, 0, 10, 10))}, DbuBox(0, 0, 20, 20))
        with pytest.raises(ValueError, match="query_box"):
            prepare_macro_problem(batch, LAYER, "clear", CFG, _macros()[0], data_bounds=BOUNDS)
    ...
    def test_unknown_polarity_rejected(self):
        ...
        with pytest.raises(ValueError, match="极性"):
            prepare_macro_problem(batch, LAYER, "reverse", CFG, macro, data_bounds=BOUNDS)
    ...
    def test_own_missing_from_owner_membership_rejected(self):
        ...
        member_cores = {c for c in range(macro.core_count) if segment in set(problem.segments_for_core(c).tolist())}
    ...
    def test_all_context_problem_with_empty_membership_is_legal(self):
        ...
        empty = MacroProblem(
            ...
            problem.fragmentation,
            problem.segments,
            owner_indices=np.full(problem.segments.segment_count, -1, dtype=np.int32),
            core_offsets=np.zeros(macro.core_count + 1, dtype=np.int64),
            member_segment_indices=np.empty(0, dtype=np.int32),
        )
...
class TestPersistence:
    ...
    def test_save_load_roundtrip_is_exact(self, tmp_path):
        ...
        for name in (
            ...
            assert np.array_equal(getattr(loaded.segments, name), getattr(problem.segments, name))
        ...
        assert np.array_equal(loaded.member_segment_indices, problem.member_segment_indices)
